...
The typical commands for running the pipeline on AlphaFold2, ESMFold and RoseTTAFold-All-Atom modes are shown below.

> You can run any combination of the models by providing them to the `--mode` parameter separated by a comma. For example: `--mode alphafold2,esmfold` will run the three models in parallel.
...
Given that the AlphaFold2 rely on huge databases to infer the predictions, the execution of the pipeline is recommended to take place on shared file systems so as to avoid high latency caused during staging this data. For instance, if you work on AWS, you might consider using an Amazon FSx file system.

...
class MyModel(nn.Module):
	def __init__(
		...
		for _ in range(self.num_layers):
			...
			if model_type == 'gcn':
				...
				raise ValueError('Invalid model type: you should choose model type in [gcn, gin, gine, gat]')
	...
	def forward(
		...
		assert feat.ndim == 2 and feat.size(1) == self.embedding_node.in_features
		assert eweight.ndim == 2 and eweight.size(1) == self.embedding_edge.in_features

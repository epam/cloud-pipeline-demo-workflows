...
def evaluate_regression(
	...
	return cast(float, mse), cast(float, rmse), cast(float, r2)
...
def plot_attention(
    ...
    #attention_list = attention_list[np.newaxis, :]
    ...
    for i, mol in enumerate(mol_list):
        ...
        print(highlight_at)
        highlight_atom = [highlight_at.index(x) if x>0.3 else 0 for x in highlight_at]
        print(highlight_atom)
        highlight_atoms.append(highlight_atom) 
...
def plot_attention_svg(
	...
	drawer.DrawMolecule(
		...
		highlightBonds=radius
	)
	drawer.FinishDrawing()
	svg = drawer.GetDrawingText().replace('svg:', '')

	f = open('./figures/attentions/'+path+'.svg', 'w')
	f.write(svg)
	f.close()
	cairosvg.svg2png(
		url='./figures/attentions/'+path+'.svg',
		write_to='./figures/attentions/'+path+'.png'
	)
...
def plot_svg_list(
    ...
    attention_list = np.mean(attention_list, axis=0)
    ...
    for i, mol in enumerate(mol_list):
        ...
        path = 'ESR2'+id_list[i] + '_attn_3w2o'
        plot_attention_svg(
            mol=mol,
            radius=radius,
            num_atoms=num_atoms,
